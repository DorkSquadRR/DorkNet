    ...
    ILogger<AdminController> adminLogger) : ControllerBase
{
    ...
    public sealed record WipeImportTargetsRequest(
        long[]? InventionIds,
        string[]? RoomNames);

    /// <summary>POST <c>api/admin/v1/import/wipe-targets</c> — clear the
    /// DB rows that would conflict with a zip re-import. Built for the
    /// zip importer's "Wipe duplicates first" affordance — when the
    /// admin uploads a refreshed zip after a recovery pass added more
    /// .inv bytes, the second import would otherwise: (a) reject every
    /// room with <c>duplicate_name</c>, and (b) silently auto-id every
    /// invention whose original id is already in the DB, leaving the
    /// room blob's placement refs pointing at orphans (= empty/missing
    /// inventions in-game).
    ///
    /// Wipes:
    ///   * <see cref="InventionEntity"/> rows for the listed ids + their
    ///     associated <see cref="RoomDataBlobEntity"/> rows (.inv bytes
    ///     + image thumbnails)
    ///   * <see cref="RoomEntity"/> rows matching any of the listed
    ///     names exactly + their <see cref="RoomSceneEntity"/> rows +
    ///     <see cref="RoomDataBlobEntity"/> rows (scene blobs + room
    ///     image)
    ///
    /// Skips dorms and seed-owned RR-Originals (same safety net as
    /// <see cref="HardDeleteRoom"/>) — only user-content survives the
    /// wipe path.
    ///
    /// Returns a per-room and per-invention rollup so the UI can show
    /// what landed before kicking off the actual upload.</summary>
    [HttpPost("import/wipe-targets")]
    public async Task<ActionResult> WipeImportTargets([FromBody] WipeImportTargetsRequest body)
    {
        var inventionIds = body?.InventionIds ?? Array.Empty<long>();
        var roomNames    = body?.RoomNames    ?? Array.Empty<string>();
        if (inventionIds.Length == 0 && roomNames.Length == 0)
            return BadRequest(new { error = "nothing_to_wipe" });

        // ── Inventions ────────────────────────────────────────────────
        var inventions = inventionIds.Length == 0
            ? new List<InventionEntity>()
            : await db.Inventions.Where(i => inventionIds.Contains(i.Id)).ToListAsync();
        var invBlobNames = inventions
            .SelectMany(i => new[] { i.CurrentBlobName, i.ImageName })
            .Where(n => !string.IsNullOrEmpty(n))
            .Cast<string>()
            .Distinct()
            .ToList();
        var invBlobs = invBlobNames.Count == 0
            ? new List<RoomDataBlobEntity>()
            : await db.RoomDataBlobs
                .Where(b => b.RoomId == 0 && invBlobNames.Contains(b.BlobName))
                .ToListAsync();

        // ── Rooms ─────────────────────────────────────────────────────
        // Exact-name match — same key the importer uses for its
        // duplicate-name rejection. Skip dorms + seed-owned RR-Originals
        // so a fat-finger on the wipe list can't nuke canonical content.
        var roomMatches = roomNames.Length == 0
            ? new List<RoomEntity>()
            : await db.Rooms.Where(r => roomNames.Contains(r.Name)).ToListAsync();
        var roomsToDelete = roomMatches
            .Where(r => !r.IsDormRoom
                     && !(r.IsAGRoom && r.CreatorPlayerId == PlayerService.SystemAccountId))
            .ToList();
        var roomIds = roomsToDelete.Select(r => r.Id).ToList();
        var roomScenes = roomIds.Count == 0
            ? new List<RoomSceneEntity>()
            : await db.RoomScenes.Where(s => roomIds.Contains(s.RoomId)).ToListAsync();
        var roomBlobs = roomIds.Count == 0
            ? new List<RoomDataBlobEntity>()
            : await db.RoomDataBlobs.Where(b => roomIds.Contains(b.RoomId)).ToListAsync();
        var roomImageNames = roomsToDelete
            .Select(r => r.ImageName)
            .Where(n => !string.IsNullOrEmpty(n))
            .Cast<string>()
            .Distinct()
            .ToList();
        var roomImages = roomImageNames.Count == 0
            ? new List<RoomDataBlobEntity>()
            : await db.RoomDataBlobs
                .Where(b => b.RoomId == 0 && roomImageNames.Contains(b.BlobName))
                .ToListAsync();

        db.RoomDataBlobs.RemoveRange(invBlobs);
        db.Inventions.RemoveRange(inventions);
        db.RoomScenes.RemoveRange(roomScenes);
        db.RoomDataBlobs.RemoveRange(roomBlobs);
        db.RoomDataBlobs.RemoveRange(roomImages);
        db.Rooms.RemoveRange(roomsToDelete);

        await LogAsync("wipe_import_targets", "import", 0,
            $"inventions={inventions.Count}/{inventionIds.Length} " +
            $"rooms={roomsToDelete.Count}/{roomNames.Length} " +
            $"roomBlobs={roomBlobs.Count} roomScenes={roomScenes.Count} " +
            $"invBlobs={invBlobs.Count} roomImages={roomImages.Count}");
        await db.SaveChangesAsync();

        var skippedRooms = roomMatches
            .Where(r => r.IsDormRoom
                     || (r.IsAGRoom && r.CreatorPlayerId == PlayerService.SystemAccountId))
            .Select(r => new { id = r.Id, name = r.Name, reason = r.IsDormRoom ? "dorm" : "seeded_rr_original" })
            .ToList();

        return Ok(new
        {
            inventionsDeleted = inventions.Count,
            inventionsRequested = inventionIds.Length,
            inventionsNotFound  = inventionIds.Length - inventions.Count,
            inventionBlobsRemoved = invBlobs.Count,
            roomsDeleted = roomsToDelete.Count,
            roomsRequested = roomNames.Length,
            roomsNotFound  = roomNames.Length - roomMatches.Count,
            roomScenesRemoved = roomScenes.Count,
            roomBlobsRemoved  = roomBlobs.Count + roomImages.Count,
            skippedRooms,
        });
    }

    public sealed record PurgeInventionsRequest(
        long[]? InventionIds,
        // The caller must echo back the count of ids it expects to delete.
        // Mismatch = client and server disagree about scope → refuse.
        // Same pattern as `git rm` requiring -f when files would be lost.
        int? ExpectedCount);

    /// <summary>POST <c>api/admin/v1/inventions/purge</c> — hard-delete a
    /// batch of inventions by id. Used by the zip-recovery workflow to
    /// wipe a room's referenced inventions before re-importing a refreshed
    /// zip — avoids the duplicate-id problem where the zip importer
    /// auto-generates new ids when the original is already in the DB,
    /// leaving the room blob's invention references pointing at orphans.
    ///
    /// Deletes:
    ///   * <see cref="InventionEntity"/> rows for the given ids
    ///   * Associated <see cref="RoomDataBlobEntity"/> rows for both the
    ///     invention's <c>CurrentBlobName</c> (the .inv bytes) and
    ///     <c>ImageName</c> (the thumbnail). RoomId on those rows is 0
    ///     for invention-owned blobs.
    ///
    /// Does NOT touch S3 — the importer will overwrite on re-import, and
    /// orphan S3 keys are harmless (read path lookups them by BlobName
    /// from the DB).
    ///
    /// Logs each delete to AdminActions for audit replay.
    /// </summary>
    [HttpPost("inventions/purge")]
    public async Task<ActionResult> PurgeInventions([FromBody] PurgeInventionsRequest body)
    {
        var ids = body?.InventionIds;
        if (ids is null || ids.Length == 0)
            return BadRequest(new { error = "missing_ids" });
        if (body!.ExpectedCount is not int ec || ec != ids.Length)
            return BadRequest(new { error = "expected_count_mismatch", expected = body.ExpectedCount, actual = ids.Length });

        var inventions = await db.Inventions
            .Where(i => ids.Contains(i.Id))
            .ToListAsync();

        var blobNames = inventions
            .SelectMany(i => new[] { i.CurrentBlobName, i.ImageName })
            .Where(n => !string.IsNullOrEmpty(n))
            .Cast<string>()
            .Distinct()
        ...
        var blobs = await db.RoomDataBlobs
            .Where(b => b.RoomId == 0 && blobNames.Contains(b.BlobName))
            .ToListAsync();

        db.RoomDataBlobs.RemoveRange(blobs);
        db.Inventions.RemoveRange(inventions);

        await LogAsync("purge_inventions", "invention", 0,
            $"count={inventions.Count} blobs={blobs.Count} ids=[{string.Join(",", inventions.Select(i => i.Id).Take(20))}{(inventions.Count > 20 ? ",..." : "")}]");
        await db.SaveChangesAsync();

        return Ok(new
        {
            deleted = inventions.Count,
            requested = ids.Length,
            notFound = ids.Length - inventions.Count,
            blobsRemoved = blobs.Count,
        });
    }
    ...
    public async Task<ActionResult> GetWeeklyChallengeRewardOptions()
    {
        ...
        static object Option(
            ...
            string equipmentModificationGuid = "") => new
        {
            Kind = kind,
        ...
        var equipment = rows
            .Select(i => StoreService.TryGetEquipmentPayload(i.Slug, out var prefab, out var mod)
                ? Option("equipment", i, equipmentPrefabName: prefab, equipmentModificationGuid: mod)
                : null)
            .Where(o => o is not null)
            .ToArray();
    ...
    /// category. The watch's pinned-cheer picker (see
    /// <see cref="DorkNet.Server.Controllers.API.PlayerCheer.PlayerCheerController.SetSelectedCheer"/>)
    /// only lets a player pin a category they've actually received — this
    /// unlocks every badge for them without having to wait on real
    /// peer cheers.
    ///
    /// Inserts come from system account id=1 (the seed admin). Idempotent
    /// on (FromPlayerId, TargetPlayerId, Type) — re-running just fills in
    /// whichever categories aren't there yet. Does NOT award XP (these
    /// are admin-injected, not real peer interactions), so the player's
    /// level isn't inflated.</summary>
    [HttpPost("players/{id:long}/cheers/unlock")]
    public async Task<ActionResult> UnlockCheers(long id)
    {
        var p = await db.Players.FirstOrDefaultAsync(x => x.Id == id);
        if (p is null) return NotFound();

        // RoomieCheerType enum mirror — see CheerEntity.Type docblock.
        var allTypes = new[] { 0, 1, 2, 3, 4, 5 };
        var existingTypes = await db.Cheers
            .Where(c => c.TargetPlayerId == id && c.TargetRoomId == 0
                     && c.TargetPhotoId == 0 && c.TargetInventionId == 0)
            .Select(c => c.Type)
            .Distinct()
            .ToListAsync();
        var missing = allTypes.Except(existingTypes).ToList();
        foreach (var t in missing)
        {
            db.Cheers.Add(new CheerEntity
            {
                FromPlayerId   = 1,   // system / seed-admin account
                TargetPlayerId = id,
                Type           = t,
            });
        }
        await LogAsync("unlock_cheers", "player", id,
    ...
    /// <summary>POST <c>api/admin/v1/storeitems/image/upload</c> —
    /// attachable catalog artwork. Stored as a global image blob so the
    /// admin UI and the watch can resolve the same ImageName through
    /// img.* / cdn.* without admins hand-copying filenames.</summary>
    [HttpPost("storeitems/image/upload")]
    [DisableRequestSizeLimit]
    [RequestFormLimits(MultipartBodyLengthLimit = 25_000_000)]
    public async Task<ActionResult> UploadStoreItemImage([FromForm(Name = "file")] IFormFile? file)
    {
        ...
        var imageName = $"store_{hash[..24]}{ext}";
        ...
        if (existing is null)
        {
            ...
            await LogAsync("store_item_image_upload", "storeitem", 0, $"image={imageName} bytes={bytes.Length}");
            await db.SaveChangesAsync();
        }
