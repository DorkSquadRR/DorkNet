    ...
    ILogger<MessagesController> logger) : ControllerBase
{
        ...
        [FromForm(Name = "roomInstanceId")] long roomInstanceId)
    {
        ...
        // Resolve which room the invite is for. Two code paths:
        //
        ...
        //   roomId*100_000 — synthetic, never registered. We have to
        //   resolve which Room they're in via PlayerPresenceService
        //   (heartbeats keep this up to date) so we can stamp RoomId
        //   on the message and the recipient's /goto/invite/{N} path
        //   knows where to send them. No ownership check applies for
        //   public rooms.
        long resolvedRoomId;
        string roomName;
        ...
        {
            // Public-room path. Trust the sender's current presence —
            // the watch only fires SendInvite from the social menu
            // while you're in a room, and heartbeats refresh presence
            // every few seconds. We DON'T trust the raw roomInstanceId
            // from the form because it's user-controlled and a
            // malicious mod could send a fabricated one.
            var senderPresence = presence.GetRoom(Me);
            if (senderPresence is null || senderPresence.RoomId <= 0)
            {
                logger.LogWarning(
                    "[invite] from={From} to={To} instance={Instance}: no private instance AND no presence room — rejecting",
                    Me, playerId, roomInstanceId);
                return BadRequest(new { error = "sender_not_in_a_room", roomInstanceId });
            }
            resolvedRoomId = senderPresence.RoomId;
            roomName = !string.IsNullOrWhiteSpace(senderPresence.Name)
                ? senderPresence.Name
                : await db.Rooms
                    .Where(r => r.Id == senderPresence.RoomId)
                    .Select(r => r.Name)
        ...
        // Only pin the invitee row for PRIVATE instances — public-room
        // invites have no registered instance to track invitees against.
        // GoToInvite handles the missing-mapping case by falling through
        // to the message's RoomId + BuildResponseAsync(roomName).
        if (inst is not null)
        {
    ...
    /// the response shape is <c>[ {AccountId, Online}, ... ]</c>.
    ///
    /// Both <c>favoriteFriendOnlineStatus</c> and the more general
    /// <c>onlineStatus</c> URLs alias to this handler since they
    /// share the wire shape; pre-2026-05-25 the onlineStatus route
    /// was wrongly aliased to GetMessages and returned the inbox
    /// list instead, breaking the friends-online HUD.</summary>
