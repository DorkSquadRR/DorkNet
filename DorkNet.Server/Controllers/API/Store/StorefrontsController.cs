    ...
    DomainConfig domain) : ControllerBase
{
    ...
    /// <summary>GET <c>api/storefronts/v1/p2p/betaEnabled</c> —
    /// raw Boolean gate for player-to-player sales. Returning false
    /// disables the beta affordance without lying about catalog state.</summary>
    [HttpGet("api/storefronts/v1/p2p/betaEnabled")]
    [AllowAnonymous]
    public IActionResult P2PBetaEnabled() => Content("false", "application/json");

    /// <summary>GET <c>api/storefronts/v1/trialInvention/duration</c>.
    /// The 2020.12 client's public promise projects the Duration
    /// property from this object, so the wire body is not a bare int.</summary>
    [HttpGet("api/storefronts/v1/trialInvention/duration")]
    [AllowAnonymous]
    public IActionResult TrialInventionDuration() => Ok(new
    {
        Duration = Math.Max(0, config.GetValue("Store:TrialInventionDurationSeconds", 300)),
    });
