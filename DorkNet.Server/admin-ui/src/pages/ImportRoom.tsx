...
import { ApiError, uploadZipInChunks, type UploadProgress } from '../lib/api';
...
export function ImportRoom() {
  ...
  const [normalizeBlobs, setNormalizeBlobs] = useState(false);
          ...
          {stage === 'server' && (
            <>
              ...
              <p className="text-[11px] text-ink-400 mb-2">
                ...
                Large archives can take 30s+; we'll show the per-room result table when it finishes.
              </p>
        ...
        <div className="card !p-4 mb-4 space-y-3">
            ...
            <button onClick={upload} disabled={busy || !ownerId || !chromium} className="btn-primary text-xs">
              {stage === 'uploading' ? `Uploading… ${uploadProgress.percent}%`
                ...
                : (() => {
                    ...
                    return `Import ${selectedRoomsCount} room${selectedRoomsCount === 1 ? '' : 's'} · ${selectedInvCount} invention${selectedInvCount === 1 ? '' : 's'}`;
                  })()}
            </button>
          </div>

          {/* Per-import blob-normaliser toggle. Visible as a checkbox so
              the admin makes the call PER ZIP — defaults off because the
              2020 protobuf re-encode currently crashes the watch on load.
              The diagnostic Normalize() call still runs server-side either
              way and its parse-OK / parse-FAIL log line stays in import
              telemetry. */}
          <label className="flex items-start gap-2 cursor-pointer">
            ...
            <span className="text-xs leading-snug">
              <span className="text-ink-100">Re-encode .room blobs through the 2020 protobuf schema</span>
              <span className="block text-[11px] text-ink-500 mt-0.5">
                Strips non-canonical encodings from modern Rec Room exports so the 2020 watch's
                stricter parser accepts them. Off by default — the current 2020 schema bindings
                are incomplete and the re-encode tends to crash the watch on dorm/room load.
                Leave OFF unless you've verified the round-trip works for this specific zip.
              </span>
            </span>
          </label>
        </div>
