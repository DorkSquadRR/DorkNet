...
import { Plus, RefreshCw, Search, Trash, Upload } from '../components/Icons';
...
const CATEGORIES = ['head', 'torso', 'legs', 'feet', 'accessory', 'hair', 'face', 'consumable', 'roomtemplate', 'weapon', 'equipment', 'tool', 'maker_pen'];
...
export function Store() {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'visible' | 'hidden' | 'limited' | 'expired' | 'missing-image'>('all');
  ...
  const [pendingBulk, setPendingBulk] = useState<null | { action: 'delete' | 'show' | 'hide'; count: number }>(null);
  ...
  const items = useMemo(() => {
    ...
    const filtered = data.filter(it => {
      ...
      if (activeFilter === 'visible' && !it.isActive) return false;
      if (activeFilter === 'hidden' && it.isActive) return false;
      if (activeFilter === 'limited' && !it.isLimitedTime) return false;
      if (activeFilter === 'missing-image' && it.imageName) return false;
  ...
  const setItemVisible = async (item: StoreItem, visible: boolean) => {
    try {
      await api(`/storeitems/${item.id}`, {
        method: 'PUT',
        body: { IsActive: visible },
      });
      toast.push(visible ? 'Item visible in store' : 'Item hidden from store', 'success');
      refresh();
    } catch (e) {
      toast.push((e as Error).message, 'error');
    }
        ...
        try {
          if (pendingBulk.action === 'delete') {
            ...
            await api(`/storeitems/${it.id}`, {
              method: 'PUT',
              body: { IsActive: pendingBulk.action === 'show' },
            });
          }
          ok++;
        } catch { failed++; }
  ...
  return (
    <div>
      <PageHeader
        title="Store catalog"
        blurb="Manage what appears in the in-game store. Hide keeps the item in the catalog but removes it from every live storefront; delete is only for cleanup."
        actions={<>
      ...
      <div className="card !p-3 mb-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-[11px]">
          {(['all', 'visible', 'hidden', 'missing-image', 'limited', 'expired'] as const).map(f => (
            <button
              key={f}
            ...
            >
              {f === 'all' ? 'All' :
               f === 'visible' ? 'Visible in store' :
               f === 'hidden' ? 'Hidden' :
               f === 'missing-image' ? 'Needs picture' :
      ...
      {selected.size > 0 && (
        <div className="card !p-2 mb-4 border-brand-500/40 bg-brand-500/5 flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex gap-1">
            <button
              onClick={() => setPendingBulk({ action: 'show', count: selected.size })}
              className="btn-secondary text-xs"
              disabled={bulkBusy}
            >Show</button>
            <button
              onClick={() => setPendingBulk({ action: 'hide', count: selected.size })}
              className="btn-secondary text-xs"
              disabled={bulkBusy}
            >Hide</button>
            <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {items.map(it => (
            <StoreCard
              ...
              onToggleVisible={() => setItemVisible(it, !it.isActive)}
              ...
              <tbody className="divide-y divide-ink-800">
                {items.map(it => (
                  <tr key={it.id} className={`table-row-hover ${selected.has(it.id) ? 'bg-brand-500/5' : ''}`}>
                    ...
                    <td className="px-2 py-2"><Thumb imageName={it.imageName} size={36} label={it.displayName} category={it.category} /></td>
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      <button onClick={() => setItemVisible(it, !it.isActive)} className="btn-ghost text-xs">
      ...
      <Confirm
        ...
        title={
          ...
          pendingBulk?.action === 'show' ? `Show ${pendingBulk?.count} items` :
          `Hide ${pendingBulk?.count} items`
        }
        body={pendingBulk?.action === 'delete'
          ...
          : <><span className="font-medium text-ink-50">{pendingBulk?.count}</span> selected items will be {pendingBulk?.action === 'show' ? 'visible in the in-game store' : 'hidden from every in-game storefront'}.</>}
...
function StoreCard({
  item, selected, onToggleSelected, onEdit, onToggleVisible, onDelete,
}: {
  item: StoreItem;
  selected: boolean;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title="Edit"
      >
        <Thumb imageName={item.imageName} size={256} fill label={item.displayName} category={item.category} />
      </button>
      ...
      <div className="px-2.5 py-2 flex flex-col gap-1">
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          <div className="flex items-center gap-1">
            <button onClick={onToggleVisible} className="btn-ghost text-[11px] py-0.5 px-1.5">
...
function Thumb({
  ...
  if (!url) {
    const initial = (label || category || '?').trim().slice(0, 1).toUpperCase() || '?';
    return (
      <div
        className={`${fill ? 'w-full h-full' : ''} flex flex-col items-center justify-center bg-gradient-to-br from-ink-900 via-ink-950 to-ink-900 text-center`}
        style={fill ? undefined : { width: size, height: size }}
        title={imageName ?? 'no image'}
      >
        <span className="flex size-12 items-center justify-center rounded-full border border-ink-700 bg-ink-800/60 text-lg font-semibold text-ink-200">
          {initial}
        </span>
        <span className="mt-2 max-w-full truncate px-2 text-[10px] uppercase tracking-wide text-ink-500">
          {category || 'store item'}
        </span>
      </div>
    );
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      const res = await api<{ imageName: string; imageUrl: string; bytes: number; reused: boolean }>(
        '/storeitems/image/upload',
        { method: 'POST', formData: fd, timeoutMs: 60_000 },
      );
      setForm(current => ({ ...current, imageName: res.imageName }));
      toast.push(res.reused ? 'Picture already existed; reused it' : 'Picture uploaded', 'success');
    } catch (e) {
      toast.push((e as Error).message, 'error');
    } finally {
