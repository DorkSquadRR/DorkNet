...
// The 2020.03 StoreService exposes avatar-outfit + consumable payloads
// only (no equipment helper), so the reward picker offers those two
// kinds. Each option carries the item `slug` — the reliable handle the
// server grants on completion. (The December build keyed rewards off the
// descriptor strings alone and never actually landed the item.)
interface RewardOption {
  kind: 'avatar' | 'consumable';
  label: string;
  slug: string;
  giftDropId: number;
  avatarItemDesc: string;
...
export function Settings() {
  ...
  const allRewardOptions: RewardOption[] = [
    ...rewardOptions.avatarItems,
    ...rewardOptions.consumables,
  ];

  // Selection is keyed purely on the item slug — the one handle that
  // round-trips through save/grant. A configured reward whose slug no
  // longer matches an active store item falls back to "current" so the
  // admin can see (and re-pick) it.
  const selectedRewardValue = weekly
    ? (weekly.reward.slug
        ? (allRewardOptions.some((o) => o.slug === weekly.reward.slug)
            ? `slug:${weekly.reward.slug}`
            : 'current')
        : 'none')
    : 'none';
  ...
  const selectedRewardOption = selectedRewardValue === 'current'
    ? weekly?.reward ?? null
    : allRewardOptions.find((o) => `slug:${o.slug}` === selectedRewardValue) ?? null;
  ...
  const selectWeeklyReward = (value: string) => {
    ...
    const selected = allRewardOptions.find((o) => `slug:${o.slug}` === value);
    if (!selected) return;
    updateWeeklyReward({
        ...
        <div className="card !p-5 max-w-5xl mt-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs text-ink-400">
                These are sent to the watch as the current challenge map. Players start each weekly challenge incomplete until their own progress is reported. When the week's challenges complete, the reward item is granted straight to the player's inventory.
              </p>
              <p className="mt-2 text-[11px] text-ink-500">
          ...
          <div className="mt-5 rounded-lg border border-ink-800 bg-ink-950/30 p-4">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <label className="block md:col-span-3">
                ...
                >
                  <option value="none">None (XP + tokens only)</option>
                  {selectedRewardValue === 'current' && (
                    <option value="current">Current item ({weekly.reward.slug || 'unknown'})</option>
                  )}
                  <optgroup label="Avatar items">
                    {rewardOptions.avatarItems.map((option) => (
                      <option key={`slug:${option.slug}`} value={`slug:${option.slug}`}>{option.label}</option>
                    ))}
                  </optgroup>
                  <optgroup label="Consumables">
                    {rewardOptions.consumables.map((option) => (
                      <option key={`slug:${option.slug}`} value={`slug:${option.slug}`}>{option.label}</option>
                    ))}
                  </optgroup>
                </select>
