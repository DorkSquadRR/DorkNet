...
export interface UploadProgress { loaded: number; total: number; percent: number; phase?: string }
...
export async function uploadZipInChunks<T = unknown>(
  ...
  try {
    ...
    // 2. Finalize — server kicks off the import in a background task
    //    and returns 202 immediately with a jobId. Multi-GB archives
    //    take several minutes to process and used to 524 through
    //    Cloudflare on the synchronous finalize path; the new flow
    //    lets us poll for progress without holding the HTTP connection
    //    open the entire time.
    const finalize = await api<{ jobId: string; state: string }>(
      `/rooms/zip-upload-finalize/${uploadId}`,
      {
        method: 'POST',
        body: {
    ...
    // 3. Poll status — every 3 seconds is a comfortable cadence:
    //    fast enough to feel responsive in the UI, slow enough that
    //    a 30-minute import doesn't generate hundreds of requests.
    //    onProgress doubles as the "phase text" channel during this
    //    stage by setting percent=100 and replaying the server's
    //    phase string through the `phase` property.
    const jobId = finalize.jobId;
    const pollIntervalMs = 3_000;
    const pollDeadline = Date.now() + timeoutMs;
    while (true) {
      if (Date.now() > pollDeadline)
        throw new ApiError(0, `import job ${jobId} did not complete within ${Math.round(timeoutMs / 1000)}s`);
      ...
      }>(`/rooms/zip-import-status/${jobId}`, { method: 'GET', timeoutMs: 30_000 });
      // Surface server-side phase to the UI via onProgress.
      onProgress({ loaded: file.size, total: file.size, percent: 100, phase: status.phase ?? undefined });
      if (status.state === 'done') return status.result as T;
      if (status.state === 'failed')
        throw new ApiError(0, status.error ?? `import job ${jobId} failed`);
